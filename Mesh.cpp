...
Mesh::Mesh(const wchar_t* full_path): Resource(full_path)
{
	...
	// Conversion sécurisée de std::wstring en std::string
	std::wstring wfull_path(full_path);
	std::wcout << L"Full path (wstring): " << wfull_path << std::endl; // Afficher l'entrée

	std::mbstate_t state = std::mbstate_t();
	const wchar_t* data = wfull_path.c_str();
	size_t len = 0;
	wcsrtombs_s(&len, nullptr, 0, &data, 0, &state);
	std::vector<char> buffer(len);
	wcsrtombs_s(&len, buffer.data(), buffer.size(), &data, buffer.size(), &state);
	std::string inputfile(buffer.data());
	std::cout << "Input file (string): " << inputfile << std::endl;
	...
	m_index_buffer = GraphicsEngine::get()->getRenderSystem()->createIndexBuffer(&list_vertices[0],(UINT)list_vertices.size());
}
