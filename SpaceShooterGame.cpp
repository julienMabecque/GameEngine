...
void SpaceShooterGame::render()
{
	...
	updateModel(m_current_spaceship_pos, m_current_spaceship_rot, Vector3D(5,5,5), m_list_materials);
...
void SpaceShooterGame::updateCamera()
{
	...
	m_cam_rot.m_x += m_delta_mouse_x * m_delta_time * 0.1f;
	m_cam_rot.m_y += m_delta_mouse_y * m_delta_time * 0.1f;
...
void SpaceShooterGame::updateThirdPersonCamera()
{
	...
	new_pos = new_pos + world_cam.getYDirection() * (10.0f);
...
void SpaceShooterGame::updateSpaceship()
{
	...
	if (m_spaceship_rot.m_x >= 1.57f) m_spaceship_rot.m_x = 1.57f;
	else if (m_spaceship_rot.m_x <= -1.57f) m_spaceship_rot.m_x = -1.57f;
	...
	//// Apply constant 90-degree rotation around X
	//temp.setIdentity();
	//temp.setRotationX(1.5708f); // 90 degrees in radians
	//world_model *= temp;

	m_spaceship_speed = 200.0f;
	if (m_turbo_mode)
	{
...
void SpaceShooterGame::onCreate()
{
	...
	m_sky_tex = GraphicsEngine::get()->getTextureManager()->createTextureFromFile(L"Assets\\Textures\\TychoSkymap.t5_04096x02048.jpg");
	m_sky_mesh = GraphicsEngine::get()->getMeshManager()->createMeshFromFile(L"Assets\\Meshes\\sphere.obj");
	

	m_spaceship_tex = GraphicsEngine::get()->getTextureManager()->createTextureFromFile(L"Assets\\Textures\\Toy Rocket_Toy Ship_BaseColor.png");
	m_spaceship_mesh = GraphicsEngine::get()->getMeshManager()->createMeshFromFile(L"Assets\\Meshes\\starWarsSpaceShip.obj");
...
{
	RECT rc = this->getClientWindowRect();
	m_swap_chain->resize(rc.right - rc.left, rc.bottom - rc.bottom);
	this->update();
	this->render();
...
void SpaceShooterGame::onMouseMove(const Point& mouse_pos)
{
	...
	InputSystem::get()->setCursorPosition(Point(win_size.left+(int)(width / 2), win_size.top+(int)(height / 2)));
}
